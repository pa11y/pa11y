...
const run = async options => {
	 ...
	 */
	const choosePa11yLevel = (axeImpact, issueNeedsReview) => {
		let level = axeImpactToPa11yLevel?.[axeImpact] ?? 'error';
		if (issueNeedsReview && level === 'error') {
			level = 'warning';
		}
		return level;
	};
	...
	const processIssue = (
		...
		return (nodes.length ? nodes : nodesForNodelessIssue())
			...
			.map(element => ({
				element,
				code: id,
				type: choosePa11yLevel(impact, issueNeedsFurtherReview),
	...
	async function runAxeCore({standard, rules, ignore, rootElement}) {
		...
		return [
			...violations.flatMap(issue => processIssue(issue, false)),
			...incomplete.flatMap(issue => processIssue(issue, true))
		];
	}
