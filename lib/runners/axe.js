...
const run = async options => {
	 ...
	 * @param {string} levelCapForReviewRequired Maximum level for incomplete issues (error, warning, notice)
	 * @returns {import('../pa11y').Pa11yLevel} A Pa11y level
	 */
	const choosePa11yLevel = (axeImpact, issueNeedsReview, levelCapForReviewRequired) => {
		let level = axeImpactToPa11yLevel?.[axeImpact] ?? 'error';
		if (issueNeedsReview && levelCapForReviewRequired && ! isWithinMaxLevel(level, levelCapForReviewRequired)) {
			level = levelCapForReviewRequired;
		}
		return level;
	};
	 ...
	 * @param {boolean} issueNeedsFurtherReview This axe issue is incomplete and may need review.
	 * @param {string} levelCapForReviewRequired Maximum level for incomplete issues (error, warning, notice)
	...
	const processIssue = (
		...
		issueNeedsFurtherReview = false,
		levelCapForReviewRequired
	) => {
		return (nodes.length ? nodes : nodesForNodelessIssue())
			...
			.map(element => ({
				element,
				code: id,
				type: choosePa11yLevel(impact, issueNeedsFurtherReview, levelCapForReviewRequired),
	 ...
	 */
	async function runAxeCore({standard, rules, ignore, rootElement, levelCapForReviewRequired}) {
		const {
			violations = [],
			incomplete = []
		} =
		...
		return [
			...violations.flatMap(issue => processIssue(issue, false, levelCapForReviewRequired)),
			...incomplete.flatMap(issue => processIssue(issue, true, levelCapForReviewRequired))
		];
	}
