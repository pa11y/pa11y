...
const run = async options => {
	...
	/**
	 * Standards that include AA-level rules.
	 * @type {Set<string>}
	 */
	const aaStandards = new Set(['WCAG2AA', 'WCAG2AAA', 'WCAG22AA']);

	/**
	 * Standards that include WCAG 2.2 rules.
	 * @type {Set<string>}
	 */
	const wcag22Standards = new Set(['WCAG22A', 'WCAG22AA']);
	...
	const createAxeRunOnlyTags = pa11yStandard => ({
		...
		values: [
			'wcag2a',
			'wcag21a',
			...(wcag22Standards.has(pa11yStandard) ? ['wcag22a'] : []),
			...(aaStandards.has(pa11yStandard) ? [
				'wcag2aa',
				'wcag21aa',
				...(pa11yStandard === 'WCAG22AA' ? ['wcag22aa'] : [])
			] : []),
			'best-practice'
		]
	});
