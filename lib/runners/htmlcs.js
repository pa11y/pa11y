...
runner.run = async options => {
	...
	/**
	 * Standards only supported by the axe runner.
	 * @private
	 */
	const axeOnlyStandards = ['WCAG22A', 'WCAG22AA'];
	...
	if (axeOnlyStandards.includes(options.standard)) {
		throw new Error(
			`The ${options.standard} standard is only supported by the axe runner`
		);
	}
