...
function runTests (opts, context, done) {
    jsdom.env({
        ...
        scripts: getDomScripts(opts),
            ...
            var tests = opts.tests.filter(_.isFunction).map(prepareTest.bind(null, dom));
            async.parallelLimit(tests, opts.concurrency, function (err, results) {
...
function prepareTest (dom, test) {
    return function (next) {
        test(dom, next.bind(null, null));
    };
}
