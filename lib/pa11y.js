...
async function setPage(options, state) {
	...
	state.consoleCallback = message => {
		...
		// eslint-disable-next-line no-underscore-dangle
		options.log.debug(`Browser Console: args: ${JSON.stringify(message._args, getCircularReplacer)}`);
		// eslint-disable-next-line no-underscore-dangle
		options.log.debug(`Browser Console: location: ${JSON.stringify(message._location), getCircularReplacer}`);
...
/* detect circular references and remove them. from: https://stackoverflow.com/a/59833943/3053630 */
