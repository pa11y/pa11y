...
function testPage(browser, page, options, done) {
	...
	async.waterfall([
		...
		function(next) {
			var infoMessage = 'beforeScript cannot be used in combination with actions, ';
			infoMessage += 'ignoring beforeScript';
			if (typeof options.beforeScript !== 'function') {
				return next();
			} else if (options.actions.length) {
				options.log.info(infoMessage);
				return next();
			}
		...
		function(next) {
			...
			actions = options.actions.map(function(actionString) {
