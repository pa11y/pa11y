...
async function runPa11yTest(url, options, state) {
	...
	const puppeteerConfig = options.chromeLaunchConfig;
	puppeteerConfig.timeout = options.timeout;

	const browser = state.browser = await puppeteer.launch(puppeteerConfig);
