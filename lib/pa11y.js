...
exports.sniff = function (opts, callback) {
	...
	async.series([
		...
		function (next) {
			if (self.opts.debug) {
				var elapsedTime = self.timerEndTime - self.timerStartTime;
				self.reporter.debug('Timeout cleared (' + elapsedTime + '/' + self.opts.timeout + 'ms)');
...
function getMessages (self, next) {
	...
	self.page.evaluate(function () {
		...
		self.timerEndTime = (new Date()).getTime();
