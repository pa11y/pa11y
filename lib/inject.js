...
function injectPa11y(window, options, done) {
	...
	function processMessages(messages) {
		return messages.filter(isMessageInTestArea).map(processMessage).filter(isMessageWanted);
	}
	...
	function isMessageInTestArea(message) {
		var testArea = window.document.querySelector(options.selector);

		if (options.selector) {
			return isElementWithinTestArea(message.element, testArea);
		} else {
			return true;
		}
	}
