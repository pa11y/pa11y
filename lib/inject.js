...
function injectPa11y(window, options, done) {
	...
	var standardName;
	var errorMessage = 'Page not verified - HTML did not contain: "';
	errorMessage += options.verifyPage + '"';

	if (options.verifyPage) {
		windowHtml = window.document.documentElement.outerHTML;
	...
	if (options.rules.length > 0 && options.standard !== 'Section508') {
		...
		for (rule = 0; rule < options.rules.length; ++rule) {
			if (window.HTMLCS_WCAG2AAA.sniffs.indexOf(options.rules[rule]) > -1) {
				standardName = 'HTMLCS_' + options.standard.toString();
				window[standardName].sniffs[0].include.push(options.rules[rule]);
			} else {
	...
	function getCssSelectorForElement(element, selectorParts) {
		...
		if (isElementNode(element)) {
			identifier = buildElementIdentifier(element);
	...
	function hiddenAreasContainsElement(element, hiddenSelectors) {
		...
		for (i = 0; i < hiddenElements.length; i += 1) {
