...
function injectPa11y(window, options, done) {
	...
	if(options.rules && options.standard !== 'Section508') {
		for (var rule = 0; rule < options.rules.length; ++rule) {
			if(window.HTMLCS_WCAG2AAA.sniffs.includes(options.rules[rule])) {
				window['HTMLCS_' + options.standard.toString()].sniffs[0].include.push(options.rules[rule]);
			} else {
				reportError(options.rules[rule] + ' is not a valid WCAG 2.0 rule');
			}
		}
	}
