		...
		build: function(browser, page, options, matches) {
			...
			function waitForState(state, done) {
				page.evaluate(function(actionOptions) {
					...
					/* Validate target that should exist */
					if ((actionOptions.state === 'removed' || actionOptions.state === 'hidden') && !target) {
						return false;
					}
					...
					return Boolean(target.offsetWidth || target.offsetHeight || target.getClientRects().length);
				}, actionOptions, function(error, result) {
					options.log.debug('  … waiting ("' + result + '")');
					var wait = result;
					...
					if (wait) {
						...
						if (retryCount > maxRetries) {
							return done(new Error('Failed action: element "' + actionOptions.selector + '" failed to be ' + actionOptions.state));
						}
						setTimeout(function() {
