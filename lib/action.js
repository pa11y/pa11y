...
module.exports.actions = [
	...
	// E.g. "wait for .foo .bar to be visible"
	{
		...
		run: async (browser, page, options, matches) => {
			...
			const isAddedOrRemoved = (desiredState, targetElement) =>
				(desiredState === 'added' && targetElement) ||
				(desiredState === 'removed' && !targetElement);


			const isVisibleOrHidden = (desiredState, targetElementVisible) =>
				(desiredState === 'visible' && targetElementVisible) ||
				(desiredState === 'hidden' && !targetElementVisible);
			...
			await page.waitForFunction(
				(targetSelector, desiredState) => {
					...
					if (isAddedOrRemoved(desiredState, targetElement)) {
						return true;
					}
	...
	// E.g. "wait for .tab-panel to emit load"
	{
		...
		run: async (browser, page, options, matches) => {
			...
			try {
				...
				await page.evaluate((targetSelector, event) => {
					const target = document.querySelector(targetSelector);
					if (!target) {
						return Promise.reject(new Error('No element found'));
					}
