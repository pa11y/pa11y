...
module.exports.actions = [
	...
	// E.g. "wait for url to be https://example.com/"
	{
		...
		run: async (browser, page, options, matches) => {
			...
			try {
				/* eslint-disable no-shadow */
				await page.evaluate((property, expectedValue, negated) => {
					let valid;

					if (negated) {
						valid = window.location[property] !== expectedValue;
					} else {
						valid = window.location[property] === expectedValue;
					}
				...
				throw new Error(`Failed action: waiting for ${subject} to ${negated ? 'not be' : 'be'} ${expectedValue}`);
			}
			/* eslint-enable no-shadow */
		}
	},
