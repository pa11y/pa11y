...
function buildAction(browser, page, options, actionString) {
	...
	var actionRunner;
	if (actionBuilder) {
		actionRunner = actionBuilder.build(
			browser,
			page,
			options,
			actionString.match(actionBuilder.match)
		);
	}
		...
		build: function(browser, page, options, matches) {
			...
			return function(done) {
				page.evaluate(function(actionOptions) {
					...
					var errorMessage = 'Failed action: no element matching selector "';
					errorMessage += actionOptions.selector + '"';
