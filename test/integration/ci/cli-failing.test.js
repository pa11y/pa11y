...
describe('pa11y-ci (with a single failing URL)', () => {
	...
	it('outputs a result notice for each URL', () => {
		assert.include(global.lastResult.output, `${global.mockWebsiteAddress}/failing-1 - 1 errors`);
	});

	it('outputs error information', () => {
		assert.include(global.lastResult.output, `Errors in ${global.mockWebsiteAddress}/failing-1`);
...
describe('pa11y-ci (with multiple failing URLs)', () => {
	...
	it('outputs a result notice for each URL', () => {
		assert.include(global.lastResult.output, `${global.mockWebsiteAddress}/failing-1 - 1 errors`);
		assert.include(global.lastResult.output, `${global.mockWebsiteAddress}/failing-2 - 1 errors`);
	});

	it('outputs error information', () => {
		assert.include(global.lastResult.output, `Errors in ${global.mockWebsiteAddress}/failing-1`);
		assert.include(global.lastResult.output, 'html element should have a lang');
		assert.include(global.lastResult.output, `Errors in ${global.mockWebsiteAddress}/failing-2`);
